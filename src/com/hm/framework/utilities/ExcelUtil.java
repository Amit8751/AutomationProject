...
public class ExcelUtil {
    ...
    public static void setCellData(List<Map<String, String>> cellData){
        int rows=cellData.size();
        int cols=cellData.get(0).size();

        for(int r=0;r<rows;r++){

            Row row=wrkSheet.createRow(r);

            for(int c=0;c<cols;c++){
                Cell cell=row.createCell(c);

                Object value=cellData.get(r).get(c);
